...
| Journal card title | `text-[15px] font-semibold` | The post page's Next read rows (Journal story rows are `house-display text-[15px]`, company-story cards `text-lg` — house display face, not this tier) |
...
- on mobile, nav items collapse into a drawer that uses the same pill recipe — except on the phone-app routes (`lib/phone-chrome.ts`), where below `sm` the pill shell gives way to a compact house-skin bar and the destinations move to the fixed bottom tab bar (`components/mobile-tab-bar.tsx`: Desk / Filings / Ranking / Journal + an "Other" sheet for Themes, Sectors, Watchlists, Telegram); see "Responsive Behavior → Mobile"
...
- **the reading routes get a phone tree, not a squeezed desktop:** `/announcements`, `/themes`, `/leaderboards`, `/sectors` and the Journal index `/blog` render a house-skin phone tree below `sm` beside the untouched desktop tree (`BelowSm className="house"` / `FromSm` from `components/viewport-gate.tsx`), from one data fetch; `/desk` stays one tree whose blocks are flex-`order`ed on the phone, with the same gate per section. The Journal post page `/blog/<slug>` is also one tree: only its poster figure is gated (`app/blog/post-image.tsx` — full width from `sm`, a tappable strip that opens the full image below; same `src` in both paints so the phone fetches it once, and never `priority` on the hidden paint). Build the phone paint from the primitives in `components/mobile-card.tsx` — `MOBILE_CARD` shell, `MOBILE_LI` + `MOBILE_LINK` whole-row links, `mobileChipClass` / `MOBILE_CHIP_TAB` chips, `MobileMasthead`, `Crest`, `RankCell` — never a hand-rolled card. The Journal (`app/blog/journal-phone.tsx`) is the one editorial departure: its card shell, dividers, chip strip and Notebook rows are the primitives, but it keeps its own masthead (the Journal's display title, not `MobileMasthead`) and image-led story tiles instead of `MOBILE_LI`/`MOBILE_LINK` rows. Both trees ship in the SSR HTML until matchMedia answers, and crossing `sm` re-mounts the other tree from scratch, so state a reader can change (filters, show-all) lives in the shared parent. On the company page the phone paint sits inside a research `SectionCard`, so wrap it in `.house-tokens`, not `.house` (which would drop an opaque paper block into the card), and drop the primitives' page gutters; see "One sanctioned crossing" under "Hard rules between families".
- **phone chip strips scroll, they don't wrap:** inside a phone tree a filter / tab / sort row is one horizontal scroller (`MOBILE_CHIP_STRIP`, scrollbar hidden) and each chip is its own 44px box; "chips wrap cleanly" above is the research-surface rule.
- **the phone app chrome is route-gated:** the compact top bar (`app/(hero)/navbar.tsx`) and the fixed bottom tab bar (`components/mobile-tab-bar.tsx`) replace the pill navbar + hamburger below `sm` only on the routes listed in `lib/phone-chrome.ts` (`isPhoneAppRoute`, trailing slash tolerated): the four direct tabs (`PHONE_TABS`) and the destinations behind the "Other" tab (`PHONE_MORE_LINKS`, which light that tab via `isPhoneMoreRoute`). Add a route there, not in the components.
