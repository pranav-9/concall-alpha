...
- on mobile, nav items collapse into a drawer that uses the same pill recipe — except on the five phone-app routes (`lib/phone-chrome.ts`), where below `sm` the pill shell gives way to a compact house-skin bar and the destinations move to the fixed bottom tab bar (`components/mobile-tab-bar.tsx`); see "Responsive Behavior → Mobile"
