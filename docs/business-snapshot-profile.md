...
16 September 2026. Other company records were not changed.
...
background draft; use `?company=CARTRADE` for CarTrade. The drafts and content
audit are in `data/business-profile-drafts/`; both pilot backgrounds are now
published. The JSON envelopes retain the original review draft for audit.
...
Checks: `npx tsx tests/business-profile.test.ts`, `npm run typecheck`, and targeted
ESLint on the changed files. Pipeline checks: from `concallyser`, run
`.venv/bin/python -m pytest tests/test_business_profile.py -q`.
