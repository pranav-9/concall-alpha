...
// silently stops clamping. Scans every static class string in app/ and
// components/ for a line-clamp-N sitting beside a display utility. Static
// strings only: classes split across cn() arguments aren't seen here. The
// compiled, rendered check for the Exchange Desk feed lives in
...
for (const dir of ["app", "components"]) {
  for (const file of tsxFiles(join(ROOT, dir))) {
    for (const m of readFileSync(file, "utf8").matchAll(/["'`]([^"'`\n]*\bline-clamp-\d[^"'`\n]*)["'`]/g)) {
      ...
      if (classes.some((c) => /^line-clamp-\d+$/.test(c)) && classes.some((c) => DISPLAY.has(c))) {
        offenders.push(`${file.slice(ROOT.length)}: "${m[1]}"`);
      }
    }
  }
}
