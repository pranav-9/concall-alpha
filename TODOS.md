...
- [x] **Footer column is still headed "Learn"** — DONE 2026-09-13 (renamed to "Connect" in the visibility push, `feat/telegram-visibility`). (XS, P4) — the Telegram link sits under it after the X link. A design pass suggested renaming the eyebrow to "Connect" (fits X + Telegram + Submit Request). Decide when the footer is next touched. Start at: `components/site-footer.tsx` `learnLinks`.
