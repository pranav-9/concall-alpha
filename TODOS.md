...
## Business Snapshot split-card hero + `what_changed.stat` — deferred review findings (2026-09-20)

From the /ship review of `ui/business-snapshot-hero` (testing, maintainability, security, performance, API-contract and design specialists + Claude/Codex adversarial), plus the pipeline half `feat/business-profile-change-stat` in concallyser. Load-bearing items were fixed in those PRs: a malformed `stat` is now dropped on its own instead of withholding the whole change, the Phase 3/9 models reject `stat` (only a reviewed draft promoted with `--portal-renders-stat` may carry one), a null stat is dropped on promotion, ASCII-only digits and a newline ban in the value grammar, AA contrast for quiet text on the tinted surfaces, sign-neutral colour for a negative number, and a period-grammar check before the caption shows an arrow. What is left:

- [ ] **Phase 9 dumps `"metrics": null` for facts without metrics** (S, P3) — pre-existing, from the fact-metrics PR, and the same failure class as the stat null fixed here. Phase 9 stores `snapshot.model_dump(mode="json")`, which keeps defaults, so a fact with no metrics is written with `metrics: null`. `business_profile.schema.json` (`type: array`) and the portal's `metrics: z.array(...).optional()` both reject null, so `normalizeBusinessProfile` withholds the whole `business_facts` block for any Phase 9 company. Fix either side: omit None in `BusinessFact` (concallyser `profile_contract.py`) or make the zod field `.nullable()`; add a test that dumps a `BizSnapshotPhase9` and validates its profile keys against the JSON schema. Phase 3 is unaffected (`exclude_unset`).
- [ ] **Nothing checks a stat's number against the change text** (S, P3) — `section_review_prepare.py` `figures()` skips a bare integer under 100 and `stat.unit` is a separate key, so `+17 pts` is invisible to the unsourced-figure sweep, and no check asks that the digits occur in `headline` or `text`. The Step 6 brief now tells the reviewer to verify it by hand. Do this before the first stat is promoted: join value + unit before `figures()` and add an advisory "digits appear in the change text" check.
- [ ] **A strict reader turns every future optional key into a deploy-order hazard** (M, P4, INVESTIGATE) — `businessChangeSchema` is `.strict()`, so a portal bundle from before a new optional `what_changed` key withholds the whole change block when data carrying it lands (rollback, an old preview deployment reading prod Supabase). Consider `.strip()` on read with the strict schema kept for the producer gate and the drafts test; that gives up "reject what the schema rejects" on the reader, so it needs a product call.
- [ ] **Single-panel Business hero has no reading measure on wide screens** (S, P4) — pre-existing shape, made more visible by the padding change. Most companies carry no `what_changed`, so "The business" is one full-width panel; the 13px "More about the business" paragraph runs about 200 characters a line at 1440px (about 100 in the two-column state, past the 75-character measure). Cap the paragraphs at about `max-w-[72ch]`, or lay the lone panel out in two columns from `lg`.
- [ ] **Disclosure summaries are 32px tall on a phone-first page** (S, P4) — `summaryClass` in `business-company-background.tsx` is `min-h-8`; the portal's `TOUCH_TARGET` (44px hit area via `::before`) is used on the sectors, leaderboards and the helpfulness footer. Adding it here needs a 390px overlap check in the dense timeline and facts columns, where adjacent summaries are a few px apart.

## Company Announcements house skin — deferred review findings (2026-09-19)

From the /ship review of `fix/announcements-tab-house-skin` (testing + maintainability + performance + design specialists + Claude/Codex adversarial + Codex structured). Load-bearing items were fixed in that PR: the house palette now covers the two summary cards, the md date column fits en-IN's "Sept", the line-clamp guard is prefix-aware, and `.house-tokens` is written into the design-system doc. These were judged not worth the churn now, or sit outside this repo.

- [ ] **Order value can fall outside the 2-line phone clamp** (S, P2) — `exchange_desk_classify.py:634-636` (and 681-683) append ` (~₹N cr)` to the END of an order-win summary when the LLM text doesn't already mention "cr". Phone rows now clamp to 2 lines (`PhoneUpdateRow` in `app/desk/desk-exchange-updates.tsx`), and on 2026-09-19 that hid the tag on 4 of 218 /announcements rows (e.g. "…Madhya Pradesh Power Transmission Co. Ltd. (~₹135 cr)"). Desktop's one-line `truncate` hid it before this PR too. Decided to ship the clamp and move the value where no clamp can cut it: prepend the tag in the pipeline, or render `details.order.value_cr` as a chip beside the impact pill (the loaders would need to select it). Start at: `concallyser/scripts/exchange_desk_classify.py` (outside this repo), or the selects in `lib/exchange-desk.ts`.
- [ ] **tailwind-merge 3.x strips Tailwind v3's `outline` from focus rings** (S, P2) — pre-existing, app-wide. tailwind-merge 3.6 follows Tailwind v4 rules, so `cn()` treats `focus-visible:outline` and `focus-visible:outline-2` as a conflict and keeps only `outline-2`. On Tailwind 3.4, `outline` is what sets `outline-style: solid`, so every ring composed through `cn` (`ROW_FOCUS`, `MOBILE_FOCUS`, `MOBILE_ROW`, `mobileChipClass`) falls back to the browser's default outline style instead of the designed solid 2px teal. Reproduce: `twMerge("focus-visible:outline focus-visible:outline-2 focus-visible:outline-[var(--signal)]")` drops `focus-visible:outline`. Fix: pin tailwind-merge 2.x (Tailwind v3 rules), then re-check class lists that leaned on v4 merge semantics. Start at: `lib/utils.ts`.
- [ ] **shadcn Select value clamp cancelled by `flex`** (XS, P4) — `components/ui/select.tsx:40` puts `*:data-[slot=select-value]:line-clamp-1` beside `*:data-[slot=select-value]:flex` (a class list written for Tailwind v4). Under v3 the flex is emitted after the clamp, so the value never clamps. It's the one KNOWN entry in `tests/line-clamp-display.test.ts`, which fails once this is fixed, so drop the entry then.
- [ ] **`.house-tokens` sets `color`, so a `text-*` on the same element loses** (XS, P4) — `.house, .house-tokens` is emitted after the utilities at equal specificity, same as the `.house` background the `globals.css` comment describes. Harmless today (the wrapper carries no `text-*`), but `house-tokens text-muted-foreground` would silently render in ink. Either move `color: var(--ink)` to the `.house` paint rule (everything inside the company section sets its own colour), or say so in that comment.
...
- [ ] **Two `<h1>`s in the server HTML on the five dual-tree routes** (XS, P4) — `MobileMasthead` (or the Journal's own masthead in `app/blog/journal-phone.tsx`, added 2026-09-22) + the desktop header both render until matchMedia unmounts one; `display:none` keeps the loser out of the a11y tree once CSS lands, so it only bites no-JS/reader-mode clients and crawlers that don't render. Fix if it shows up in Search Console: demote the phone title to `role="heading"` or emit the desktop h1 only. Start at: `components/mobile-card.tsx` `MobileMasthead`, and the Journal's own `<h1>` in `app/blog/journal-phone.tsx`.
...
- [ ] **Journal card caption is `text-sm` while the post body is `text-[17px]`** (XS, P4) — a 14px-vs-17px seam at the end of every post (body went 15px → 17px on 2026-09-23, so the step is wider now); intentional demotion for now. Start at: `components/telegram-join-card.tsx`.
