...
- **Phone layouts for wide boards live in the same component as the table** (`components/score-board-table.tsx`, `app/sectors/page.tsx`): a `<ul>` below `lg`, the `<table>` from `lg`, both server-rendered and CSS-toggled, then the hidden one unmounted via `hooks/use-min-width.ts` once matchMedia has answered (`null` on the server so hydration matches). See "Responsive Behavior → Mobile" in [docs/portal-design-system.md](docs/portal-design-system.md). Server-rendered sections use the same gate through `components/viewport-gate.tsx` (`BelowSm` / `FromSm`) — `/desk` is one tree whose blocks are flex-`order`ed on the phone (change-first: Featured → activity → ranking), never a second page tree. `/announcements`, `/themes`, `/leaderboards` and `/sectors` (2026-09-13) paint a house-skin phone tree beside their untouched desktop tree (`BelowSm className="house"` / `FromSm`) from one data fetch — the SSR HTML carries both until matchMedia answers (deliberate: no layout shift, one fetch; a later breakpoint crossing re-mounts the other tree from scratch, so keep shared state in the parent, not the paint) — built on the primitives in `components/mobile-card.tsx` (card, whole-row link, chip grammar, masthead, crest). The phone "app" chrome — compact top bar in `app/(hero)/navbar.tsx` + `components/mobile-tab-bar.tsx` in the root layout — is keyed on those five routes via `lib/phone-chrome.ts`; add a route there, not in the components.
