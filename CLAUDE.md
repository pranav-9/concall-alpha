...
- The Guidance section (`guidance-history-section.tsx`, id `guidance-history`) is verdict-first: "Do they keep their word?" card + resolved track record, then "What to watch" — the top 3 live commitments as ranked cards with the rest collapsed (ranked by `compareMaterialityKeys` in `verdict.ts`: deadline FY → consolidated before segment → **what just moved** → revenue before margin → quarter precision → trail depth → key; "moved" is read off the derived trail direction, not the producer's status flag, because Phase 6 routinely leaves a revised thread on `active`; no per-company curation) — then a Guided / Delivered / Outcome table of resolved ones, truncated to 5 with a "show all" — that table sorts by **latest mention** (not horizon) and puts a miss ahead of a win on a tie, because met-first truncation hid the misses on the one section that exists to surface them. Every number and sentence is templated in `lib/guidance-tracking/verdict.ts` from the payload — the Delivered column reads ONLY the outcome trail step (`met`/`missed`), never a number parsed out of `status_reason` prose. Per-thread trails stay thread-style inside the row Drawer; don't reintroduce comparison cards. A revision on a commitment whose horizon is still ahead is **live**, not graded (shared with the Overview's Walk-the-talk count via `isGradedForTier`).
