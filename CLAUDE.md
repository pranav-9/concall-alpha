...
- The Guidance section (`guidance-history-section.tsx`, id `guidance-history`) shows **two verdicts** when the snapshot carries the forward-strength layer (`details.forward_strength` / `details.strategy_narrative`, from `/guidance-deep-track`; schema `/schemas/guidance_strength_v1.json`, parsed by `parseForwardStrength`/`parseStrategyNarrative` in `lib/guidance-snapshot/types.ts`). Order then: (1) **"How strong is the guidance right now?"** — the forward-book verdict, ambition × evidence pills + the live-book order-backed/asserted/aspiration split — beside **"The strategy behind the numbers"** (the single bet + one implied lever); (2) **"What to watch"** — the top 3 live commitments as ranked cards, rest collapsed (ranked by `compareMaterialityKeys` in `verdict.ts`: deadline FY → consolidated before segment → **what just moved** → revenue before margin → quarter precision → trail depth → key; "moved" is read off the derived trail direction, not the producer's status flag, because Phase 6 routinely leaves a revised thread on `active`; no per-company curation); (3) **"Should you believe any of it?"** — the credibility verdict + resolved track record; (4) the Guided / Delivered / Outcome table of resolved ones, truncated to 5 with a "show all" — sorted by **latest mention** (not horizon), a miss ahead of a win on a tie, because met-first truncation hid the misses on the one section that exists to surface them. Snapshots WITHOUT the forward-strength blocks keep the original **credibility-first** order (verdict card + track record on top, then what-to-watch). Ambition (`ambitious`/`measured`/`conservative`) and the two strength narratives come from the deep-track producer; the credibility tier, the live-state chips, and every resolved number are still templated in `lib/guidance-tracking/verdict.ts` from `guidance_items` — the Delivered column reads ONLY the outcome trail step (`met`/`missed`), never a number parsed out of `status_reason` prose. Per-thread trails stay thread-style inside the row Drawer; don't reintroduce comparison cards. A revision on a commitment whose horizon is still ahead is **live**, not graded (shared with the Overview's Walk-the-talk count via `isGradedForTier`).
