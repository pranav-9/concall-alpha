...
- **Phone layouts for wide boards live in the same component as the table** (`components/score-board-table.tsx`, `app/sectors/page.tsx`): a `<ul>` below `lg`, the `<table>` from `lg`, both server-rendered and CSS-toggled, then the hidden one unmounted via `hooks/use-min-width.ts` once matchMedia has answered (`null` on the server so hydration matches). See "Responsive Behavior → Mobile" in [docs/portal-design-system.md](docs/portal-design-system.md). Server-rendered sections use the same gate through `components/viewport-gate.tsx` (`BelowSm` / `FromSm`) — `/desk` is one tree whose blocks are flex-`order`ed on the phone (change-first: Featured → activity → ranking), never a second page tree; its phone chrome (compact top bar in `app/(hero)/navbar.tsx`, `components/desk-mobile-tab-bar.tsx` in the root layout) is keyed on `pathname === "/desk"`.
